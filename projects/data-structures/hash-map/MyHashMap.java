...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    public V get(K key) {
        // TODO
        int index = Math.abs(key.hashCode()) % getTableSize();
    ...
    private void rehash() {
        ...
        numEntries = 0;
        primeIndex++;
        Entry<K, V>[] a = new Entry[getTableSize()];
        for(int i = 0; i < getTableSize(); i++) {
            ...
            while(currentNode != null) {
                int index = Math.abs(currentNode.key.hashCode()) % getTableSize();
                a[index] = currentNode;
                numEntries++;
                currentNode = currentNode.next;
            }
